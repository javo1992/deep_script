	...
	function Autorizar_factura_o_liquidacion($parametros)
	{
		...
		*/
	    ...
	    }else
	    {
			...
			WHERE liquidacion_compras.id_factura = '".$parametros['fac']."'";		
	    }
		...
		// print_r($sql);die();
			...
			//codigo verificador 
				...
				$iva_porc = $this->conn->datos($sql3,$parametros['empresa']);
				if(count($iva_porc)>0)
				{
			...
			foreach ($detalle as $key => $value) {
				if($value['Cod_Bar'] !='' or $value['Codigo']!='')
				{
					...
					else
					{
					    ...
					    $sql3 = "SELECT * FROM porcentaje_iva WHERE porcentaje = ".$val_iva." AND '".date('Y-m-d')."' between fecha and fechafin";
						$iva_porc = $this->conn->datos($sql3,$cabecera['item']);
						if(count($iva_porc)>0)
						{
 	    ...
 	    if(file_exists($certificado_1.$p12))
	       {
	       	// print_r("java -jar ".$firmador." ".$nom_doc.".xml ".$url_generados." ".$url_firmados." ".$certificado_1." ".$p12." ".$pass);die();
	        	...
	        	exec($this->JDK8."java -jar ".$firmador." ".$nom_doc.".xml ".$url_generados." ".$url_firmados." ".$certificado_1." ".$p12." ".$pass, $f);
	        	...
	        	if(count($f)<6)
		 		...
		 		{		 			
		 			$respuesta = array('0'=>'No se pudo firmar el xml');
		 			return $respuesta;          
		        }
