...
class facturacionM
{
	...
	function linea_liquidacion_all($id,$id_empresa)
	{
	...
	function cliente_liquidacion($id,$id_empresa)
	{
	...
	function buscar_liquidacion($empresa,$numero=false)
	{
		$sql= "SELECT id_factura as 'id',num_factura as 'num',fecha,C.nombre,estado_factura  
	...
	function guia_remision_x_factura($Idfactura)
	{
		$sql = "SELECT * FROM guia_remision 
		WHERE id_empresa = '".$_SESSION['INICIO']['ID_EMPRESA']."' ";
		if($Idfactura)
		{
			$sql.=" AND id_fac_interna = '".$Idfactura."'";
		}
		$respuest  = $this->db->datos($sql,$_SESSION['INICIO']['ID_EMPRESA']);
	    return $respuest;
	}
	...
	{
		$sql = "SELECT * 
		FROM codigos_secuenciales
		WHERE id_empresa = '".$_SESSION['INICIO']['ID_EMPRESA']."' ";
		if($query)
		{
			$sql.=" AND detalle_secuencial like '%".$query."%'";
		}
			$respuest  = $this->db->datos($sql,$_SESSION['INICIO']['ID_EMPRESA']);
	    return $respuest;
	}
