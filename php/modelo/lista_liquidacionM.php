...
class lista_liquidacionM
{
	...
	function delete_lineas_liquidacion($empresa,$id,$factura)
	{
		$sql = "DELETE FROM lineas_liquidacion WHERE 1=1";
		if($id)
		{
	...
	{
		$sql = "DELETE FROM liquidacion_compras WHERE id_empresa = '".$empresa."' ";
		if($id)
		{
	...
	{
		$sql= "SELECT DISTINCT serie FROM liquidacion_compras WHERE serie <> '' AND id_empresa='".$_SESSION['INICIO']['ID_EMPRESA']."'";
		$result = $this->db->datos($sql,$_SESSION['INICIO']['ID_EMPRESA']);
	    return $result;
	}
	...
	{
		$sql= "SELECT id_lineas,producto,cantidad,LF.precio_uni,LF.iva,foto,total,subtotal,descuento,observacion FROM lineas_liquidacion LF 
	...
	{
		$sql= "SELECT * FROM lineas_liquidacion LF 
	...
	{
		$sql= "SELECT id_lineas as 'id',producto, precio_uni as 'precio',cantidad,iva,descuento,porc_descuento,total,subtotal FROM lineas_liquidacion 
	...
	function cliente_factura($id,$id_empresa)
	{
		...
		FROM liquidacion_compras 
		INNER JOIN cliente C ON liquidacion_compras.id_cliente = C.id_cliente
		INNER JOIN empresa ON liquidacion_compras.id_empresa = empresa.id_empresa
		WHERE liquidacion_compras.id_factura = '".$id."'";
		$result = $this->db->datos($sql,$id_empresa);
	    return $result;
	...
	function numero_factura($empresa)
	{
		...
		(
			SELECT CAST(num_factura as INT) as 'num' FROM liquidacion_compras WHERE id_empresa ='".$empresa."'
		) as o";
