...
if(isset($_GET['buscar_cliente_x_ci']))
{
	...
	echo json_encode($controlador->buscar_cliente_x_ci($query,$tipo));
}
...
class clienteC
{
 ...
 //        return  $cliente;
	...
	function editar_cliente($parametros)
	{
		...
		$detalle = $parametros['txt_tc'].'_SERIE_'.str_replace('-','', $_SESSION['INICIO']['SERIE']);
		...
		$vali = $this->factura->validar_mas_series($parametros['txt_tc'].'_SERIE_');
		    	...
		    	if(count($vali)>0)
			{
			    	...
			    	$datosSecuWhere[0]['dato'] = $numero[0]['id_secuenciales'];

			    	$this->factura->update('codigos_secuenciales',$datosSecu,$datosSecuWhere,$_SESSION['INICIO']['ID_EMPRESA']);
			}

		if(isset($parametros['txt_tc']) && $parametros['txt_tc']=='FA')
		{	
    		    // print_r($numero);die();	    
		    $re = $this->factura->add('facturas',$datosF,$_SESSION['INICIO']['ID_EMPRESA']);
		    // print_r($re);
		    $FA = $this->factura->buscar_facturas($_SESSION['INICIO']['ID_EMPRESA'],$new_num,$_SESSION['INICIO']['ID_USUARIO']);
		}else
		{
			// print_r($datosF);die();
			if($parametros['txt_ci']=='9999999999999')
			{
				return -3;
			}else{
			$re = $this->factura->add('liquidacion_compras',$datosF,$_SESSION['INICIO']['ID_EMPRESA']);
		    	// print_r($re);
		    	$FA = $this->factura->buscar_liquidacion($_SESSION['INICIO']['ID_EMPRESA'],$new_num,$_SESSION['INICIO']['ID_USUARIO']);
		    }
		}
	...
	function buscar_cliente_x_ci($query,$tipo)
	{
		$ci = false;
		if (is_numeric($query)) {
		   $ci = $query;
		...
		foreach ($nombres as $key => $value) {
			$label = $value['ci_ruc'];
			if($tipo==1){$label = $value['nombre'];}
			$lista[] = array('value'=>$value['id'],'label'=>$label,'telefono'=>$value['telefono'],'email'=>$value['mail'],'direccion'=>$value['direccion'],'nombre'=>$value['nombre'],'razon'=>$value['razon'],'ci'=>$value['ci_ruc']);
		}
		// print_r($lista);die();
        return  $lista;
	}
				...
				<div class="card-body">
					...
					<table class="table table-sm mt-2 mb-4">
						<tbody>
							<tr>
								<th>Nombre</th>
								<td style="white-space: break-spaces;">'.$value['nombre'].'</td>
							</tr>
							<tr>
								<th>Razon social</th>
								<td style="white-space: break-spaces;">'.$value['razon'].'</td>
							</tr>
							<tr>
								<th>CI / RUC</th>
								<td style="white-space: break-spaces;">'.$value['ci_ruc'].'</td>
							</tr>
							<tr>
								<th>Email</th>
								<td style="white-space: break-spaces;">'.$value['mail'].'</td>
							</tr>
							<tr>
								<th>Telefono</th>
								<td style="white-space: break-spaces;">'.$value['telefono'].'</td>
							</tr>
							<tr>
								<th>Estado</th>';
								if($value['estado']=='A')
							...
							<tr>
								<th>Direccion</th>
								<td style="white-space: break-spaces;">'.$value['direccion'].'</td>
							</tr>
		...
		foreach ($datos as $key => $value) {
			...
			<div class="card shadow mb-2">							
				<div class="card-body">
					...
					<table class="table table-sm mt-2 mb-4" style="white-space:pre-line">
						<tbody>
							<tr>
								<th>Nombre</th>
								<td style="white-space: break-spaces;">'.$value['nombre'].'</td>
